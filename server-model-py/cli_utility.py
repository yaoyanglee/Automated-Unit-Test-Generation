...
parser.add_argument('--whole_dir', type=str, required=True)
...
def read_files(url, cli_args):
    ...
    elif cli_args.whole_dir == "False":
        files = [f for f in os.listdir(cli_args.dir_under_test) if os.path.isfile(os.path.join(cli_args.dir_under_test, f))]
        pbar = tqdm(files, desc="Generating unit tests")

        for file_name in pbar:
            pbar.set_description(f"Generating tests for {file_name}", refresh=True)
            file_path = os.path.join(cli_args.dir_under_test, file_name)
            with open(file_path, 'r') as file:
                test_file_path = f"{cli_args.tests_dir}/test_{file_name.split('.')[0]}.py"
